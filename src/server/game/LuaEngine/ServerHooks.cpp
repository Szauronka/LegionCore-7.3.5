...
bool Eluna::OnAddonMessage(Player* sender, uint32 type, std::string& msg, Player* receiver, Guild* guild, Group* group, Channel* channel)
{
    ...
    TC_LOG_DEBUG(LOG_FILTER_ELUNA, "sender: %s\ntype: %s\nmsg: %u\nreceiver: %s", sender->GetName(), type, msg, receiver->GetName());
    Push(sender);
    Push(type);
...
// Auction House
void Eluna::OnAdd(AuctionHouseObject* ah, AuctionEntry* auction)
{
...
void Eluna::OnRemove(AuctionHouseObject* ah, AuctionEntry* auction)
{
...
void Eluna::OnSuccessful(AuctionHouseObject* ah, AuctionEntry* auction)
{
...
void Eluna::OnExpire(AuctionHouseObject* ah, AuctionEntry* auction)
{
