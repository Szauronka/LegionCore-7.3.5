...
namespace LuaMap
{
    ...
    int GetWorldObject(lua_State* L, Map* map)
    {
        ...
        switch (guid.GetHigh())
        {
            ...
            {
                Pet* pet = nullptr;
                map->GetCreature(guid)->isPet() ? pet = map->GetCreature(guid)->ToPet() : pet = nullptr;
                Eluna::Push(L, pet);
            }
                break;
            ...
            case HIGHGUID_CORPSE:
            {
                ...
                if (!PlayerList.isEmpty())
                {
                    for (Map::PlayerList::const_iterator i = PlayerList.begin(); i != PlayerList.end(); ++i)
                    {
                        if (i->getSource()->GetGUID() == guid && i->getSource()->IsCorpse())
                            corpse = i->getSource()->ToCorpse();
                    }
                }
                Eluna::Push(L, corpse);
            }
