...
namespace LuaGlobalFunctions
{
    ...
    int GetQuest(lua_State* L)
    {
        ...
        Eluna::Push(L, eQuestDataStore->GetQuestTemplate(questId));
        return 1;
    }
    ...
    {
        Eluna::Push(L, sWorld->GetGameTime());
        return 1;
    }
    ...
    int GetItemLink(lua_State* L)
    {
        ...
        std::string name = temp->GetName()->Str[locale];
    ...
    int PerformIngameSpawn(lua_State* L)
    {
        ...
        if (spawntype == 1) // spawn creature
        {
            if (save)
            {
                ...
                if (!creature->Create(eObjectMgr->GetGenerator<HighGuid::Creature>()->Generate(), map, PHASEMASK_NORMAL, entry, 0, 0, x, y, z, o))
                {
                    Eluna::Push(L);
                    return 1;
                }
        ...
        if (spawntype == 2) // Spawn object
        {
            ...
            uint32 guidLow = eObjectMgr->GetGenerator<HighGuid::GameObject>()->Generate();

            if (!object->CreateGameObject(entry, map, { x, y, z, o }, G3D::Quat(), 0, GO_STATE_ACTIVE))
            {
                Eluna::Push(L);
                return 1;
            }
            ...
            if (save)
            {
                ...
                object->SaveToDB();
                guidLow = sObjectMgr->GetGenerator<HighGuid::GameObject>()->Generate();
                ...
                eObjectMgr->AddGameobjectToGrid(guidLow, object->GetGOData());
            }
            else
                map->AddToMap(object);
    ...
    int AddVendorItem(lua_State* L)
    {
        ...
        if (!eObjectMgr->IsVendorItemValid(entry, item, maxcount, incrtime, extendedcost, type))
            return 0;

        eObjectMgr->AddVendorItem(entry, item, maxcount, incrtime, extendedcost, type, 0, false);
    ...
    int VendorRemoveItem(lua_State* L)
    {
        ...
        eObjectMgr->RemoveVendorItem(entry, item, 0);
        return 0;
    }
    ...
    int VendorRemoveAllItems(lua_State* L)
    {
        ...
        for (auto& vendorItem : items->m_items)
            eObjectMgr->RemoveVendorItem(entry, vendorItem->item, 0);
    ...
    int Ban(lua_State* L)
    {
        ...
        BanReturn result;
        result = eWorld->BanAccount(mode, nameOrIP, std::to_string(duration), reason, whoBanned);
    ...
    int SendMail(lua_State* L)
    {
        ...
        Player* receiverPlayer = eObjectAccessor()FindPlayer(ObjectGuid::Create<HIGHGUID_PLAYER>(receiverGUIDLow));
    ...
    */
    ...
    {
        TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, "Reloading creature template...");
        sObjectMgr->LoadCreatureTemplates();
        TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, "Initialize query data...");
        return 0;
    }
}
#endif
