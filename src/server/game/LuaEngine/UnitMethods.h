...
namespace LuaUnit
{
    ...
    {
        Eluna::Push(L, unit->HasUnitMovementFlag(MOVEMENTFLAG_ROOT));
        return 1;
    }
    ...
    int GetBaseSpellPower(lua_State* L, Unit* unit)
    {
        ...
        Eluna::Push(L, unit->ToPlayer()->GetBaseSpellPowerBonus());
        return 1;
    }
    ...
    */
    ...
    int GetFriendlyUnitsInRange(lua_State* L, Unit* unit)
    {
        ...
        Trinity::VisitNearbyObject(unit, range, searcher);
    ...
    */
    ...
    int SetFFA(lua_State* L, Unit* unit)
    {
        ...
        if (apply)
        {
            unit->SetByteFlag(UNIT_FIELD_BYTES_2, UNIT_BYTES_2_OFFSET_PVP_FLAG, UNIT_BYTE2_FLAG_FFA_PVP);
            for (Unit::ControlList::iterator itr = unit->m_Controlled.begin(); itr != unit->m_Controlled.end(); ++itr)
                if (Unit* unit = ObjectAccessor::GetUnit(*unit, *itr))
    ...
    int SetSanctuary(lua_State* L, Unit* unit)
    {
        ...
        if (apply)
        {
            unit->SetByteFlag(UNIT_FIELD_BYTES_2, UNIT_BYTES_2_OFFSET_PVP_FLAG, UNIT_BYTE2_FLAG_SANCTUARY);
            unit->CombatStop();
            unit->CombatStopWithPets();
        }
        else
            unit->RemoveByteFlag(UNIT_FIELD_BYTES_2, UNIT_BYTES_2_OFFSET_PVP_FLAG, UNIT_BYTE2_FLAG_SANCTUARY);
    ...
    int EmoteState(lua_State* L, Unit* unit)
    {
        ...
        unit->SetUInt32Value(UNIT_FIELD_EMOTE_STATE, (Emote)emoteId);
        return 0;
    }
    ...
    int SendChatMessageToPlayer(lua_State* L, Unit* unit)
    {
        ...
        packet.Initialize((ChatMsg)type, (Language)lang, nullptr, nullptr, msg);
    ...
    int SendUnitSay(lua_State* L, Unit* unit)
    {
        ...
        if (std::string(msg).length() > 0)
            unit->MonsterSay(msg, (Language)language, unit->GetGUID());
    ...
    int SendUnitYell(lua_State* L, Unit* unit)
    {
        ...
        if (std::string(msg).length() > 0)
            unit->MonsterYell(msg, (Language)language, unit->GetGUID());
    ...
    int CastCustomSpell(lua_State* L, Unit* unit)
    {
        ...
        if (has_bp0)
            spellValues.AddSpellMod(SpellValueMod::SPELLVALUE_BASE_POINT0, bp0);
        if (has_bp1)
            spellValues.AddSpellMod(SpellValueMod::SPELLVALUE_BASE_POINT1, bp1);
        if (has_bp2)
            spellValues.AddSpellMod(SpellValueMod::SPELLVALUE_BASE_POINT2, bp2);
        if (triggered)
            triggerFlag = TRIGGERED_FULL_MASK;

        unit->CastCustomSpell(spell, spellValues, target, triggerFlag, castItem ? castItem : nullptr, nullptr, unit->GetGUID());
        return 0;
    }
    ...
    int CastSpellAoF(lua_State* L, Unit* unit)
    {
        ...
        unit->CastSpell(Position(_x, _y, _z), spell, triggered ? TRIGGERED_FULL_MASK : TRIGGERED_NONE);
    ...
    int DealDamage(lua_State* L, Unit* unit)
    {
        ...
        if (school == MAX_SPELL_SCHOOL)
        {
            ...
            unit->SendAttackStateUpdate(HITINFO_AFFECTS_VICTIM, target, SPELL_SCHOOL_MASK_NORMAL, damage, 0, 0, VICTIMSTATE_HIT, 0);
        ...
        if (unit->IsDamageReducedByArmor(schoolmask))
            damage = unit->CalcArmorReducedDamage(unit, target, damage, NULL);
        ...
        if (!spell)
        {
            ...
            unit->CalcAbsorbResist(dmgInfo.GetVictim(), dmgInfo.GetSchoolMask(), dmgInfo.GetDamageType(), dmgInfo.GetDamage(), &absorb, &resist);
            ...
            unit->SendAttackStateUpdate(HITINFO_AFFECTS_VICTIM, target, SPELL_SCHOOL_MASK_NORMAL, damage, absorb, resist, VICTIMSTATE_HIT, 0);
            return 0;
        }
        ...
        SpellNonMeleeDamage dmgInfo(unit, target, spell, spellInfo->GetSpellXSpellVisualId(unit), spellInfo->GetSchoolMask());
    ...
    int DealHeal(lua_State* L, Unit* unit)
    {
        ...
        uint8 difficulty = Eluna::CHECKVAL<uint8>(L, 6);
    ...
    int AddThreat(lua_State* L, Unit* unit)
    {
        ...
        unit->AddThreat(victim, threat, SPELL_SCHOOL_MASK_NONE, spell ? spellInfo : nullptr);
        return 0;
    }
