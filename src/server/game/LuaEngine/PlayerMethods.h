...
namespace LuaPlayer
{
    ...
    int HasQuest(lua_State* L, Player* player)
    {
        ...
        Eluna::Push(L, player->hasQuest(quest));
        return 1;
    }
    ...
    int IsInArenaTeam(lua_State* L, Player* player)
    {
        ...
        if (type < MAX_ARENA_SLOT && player->InArena()) // FIXME: where to check if player has arena team?
            Eluna::Push(L, true);
        else
            Eluna::Push(L, false);
        return 1;
    ...
    {
        Eluna::Push(L, !player->HasFlag(PLAYER_FIELD_PLAYER_FLAGS, PlayerFlags::PLAYER_FLAGS_RESTING));
        return 1;
    }
    ...
    {
        uint32 spellId = Eluna::CHECKVAL<uint32>(L, 2);
        uint8 difficulty = Eluna::CHECKVAL<uint8>(L, 3);

        Eluna::Push(L, player->GetSpellCooldownDelay(spellId));
    ...
    {
        Eluna::Push(L, player->getDeathMatchScore()->total_kills);
        return 1;
    }
    ...
    int SendAuctionMenu(lua_State* L, Player* player)
    {
        ...
        player->GetSession()->SendAuctionHello(unit->GET_GUID(), unit->ToCreature());
        return 0;
    }
     ...
     *
     * @param [Creature] sender
     * @param [uint32] trainerId
     */
    int SendTrainerList(lua_State* L, Player* player)
    {
        Creature* creature = Eluna::CHECKOBJ<Creature>(L, 2);
        //uint32 trainerId = Eluna::CHECKVAL<uint32>(L, 3, 0); // no need
    ...
    int UnbindAllInstances(lua_State* /*L*/, Player* player)
    {
        ...
        {
            // @TODO: FIXXXXX
            // Player::BoundInstancesMap::iterator binds = player->GetBoundInstances(Difficulty(i));
            // for (auto itr = binds->second.begin(); itr != binds.end();)
            // {
    ...
    int DurabilityLossAll(lua_State* L, Player* player)
    {
        ...
        player->DurabilityLossAll(percent, inventory, false);
        return 0;
    }
    ...
    */
    ...
    int CompleteQuest(lua_State* L, Player* player)
    {
        ...
        Quest const* quest = eQuestDataStore->GetQuestTemplate(entry);
    ...
    int AddQuest(lua_State* L, Player* player)
    {
        ...
        if (player->CanAddQuest(quest, true))
        {
            ...
            player->UpdateQuestObjectiveData(quest);
        }
    ...
    int Whisper(lua_State* L, Player* player)
    {
        ...
        player->Whisper(text, (Language)lang, receiver->GetGUID());
        return 0;
    }
        ...
        for (uint32 i = 0; i < sSkillLineStore.GetNumRows(); ++i)
        {
            if (SkillLineEntry const* entry = sSkillLineStore.LookupEntry(i))
            {
                ...
                if (player->HasSkill(entry->ID))
                    player->UpdateSkillPro(entry->ID, 100, step);
            }
        }
    ...
    int AdvanceSkill(lua_State* L, Player* player)
    {
        ...
        if (_skillId && _step)
        {
            if (player->HasSkill(_skillId))
                player->UpdateSkillPro(_skillId, 100, _step);
        }
        return 0;
    }
    ...
    int Teleport(lua_State* L, Player* player)
    {
        ...
        if (player->isInFlight())
            player->m_taxi.RequestEarlyLanding();
        else
    ...
    int AddItem(lua_State* L, Player* player)
    {
        ...
        Item* item = player->StoreNewItem(dest, itemId, true, Item::GenerateItemRandomPropertyId(itemId, player->GetLootSpecID()));
        if (item)
    ...
    int SendAddonMessage(lua_State* L, Player* player)
    {
        ...
        player->WhisperAddon(message, prefix, player);
    ...
    */
    ...
    int ModifyBattlePayCredits(lua_State* L, Player* player)
    {
        ...
        uint64 getActualCredit = player->GetDonateTokens();
        uint64 calcNewCredit = getActualCredit + amount;
        player->AddDonateTokenCount(calcNewCredit);
        return 1;
    }
    ...
    int GossipAddQuests(lua_State* L, Player* player)
    {
        ...
        {
            Unit* unitSource = source->ToUnit();
            if (!unitSource)
                return 0;

            if (unitSource->HasFlag(UNIT_FIELD_NPC_FLAGS, UNIT_NPC_FLAG_QUESTGIVER))
    ...
    int SendQuestTemplate(lua_State* L, Player* player)
    {
        ...
        Quest const* quest = eQuestDataStore->GetQuestTemplate(questId);
        if (!quest)
            return 0;
    ...
    int GroupInvite(lua_State* L, Player* player)
    {
        ...
        if (success)
        {
            ...
            partyInvite.Initialize(invited, 0, false);
            invited->GetSession()->SendPacket(partyInvite.Write());
        }
