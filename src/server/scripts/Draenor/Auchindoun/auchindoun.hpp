...
static void DespawnCreaturesInArea(uint32 entry, WorldObject* object)
{
    ...
    GetCreatureListWithEntryInGrid(creatures, object, entry, 1000.0f);
    if (creatures.empty())
        return;
...
static uint32 const HostileFaction = 16;
static uint32 const FriendlyFaction = 35;
static uint32 const InvisibleDisplay = 11686;
