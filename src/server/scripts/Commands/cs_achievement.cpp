...
public:
    ...
    std::vector<ChatCommand> GetCommands() const override
    {
        ...
        {
            { "add",            SEC_ADMINISTRATOR,  true,  &HandleAchievementAddCommand,      ""},
            { "criteria",       SEC_ADMINISTRATOR,  true,  &HandleAchievementCriteriaCommand, ""},
            { "info",           SEC_ADMINISTRATOR,  true,  &HandleAchievementInfoCommand,     ""},
            { "delete",         SEC_ADMINISTRATOR,  true,  &HandleAchievementDeleteCommand,   ""}
        };
    ...
    static bool HandleAchievementCriteriaCommand(ChatHandler* handler, char const* args)
    {
        ...
        TC_LOG_DEBUG("criteria.achievement", "HandleAchievementCriteriaCommand criteriaType %i miscValue1 %i miscValue2 %i, miscValue3 %i", criteriaType, miscValue1, miscValue2, miscValue3);
    ...
    static bool HandleAchievementInfoCommand(ChatHandler* handler, char const* args)
    {
        ...
        // {
            // TC_LOG_DEBUG("criteria.achievement", "AchievementInfo achievement %u", achievement->ID);

            // if(Player* player = handler->GetSession()->GetPlayer())
            // {
                // TC_LOG_DEBUG("criteria.achievement", "AchievementInfo player %u", achievement->ID);
                // if(CriteriaProgressTree* treeProgress = player->GetAchievementMgr().GetCriteriaTreeProgressMap(achievement->CriteriaTree))
                // {
                    // TC_LOG_DEBUG("criteria.achievement", "AchievementInfo criteriaTree %u, achievement %u ChildrenTree %u ChildrenCriteria %u", achievement->CriteriaTree, achievement->ID, treeProgress->ChildrenTree.size(), treeProgress->ChildrenCriteria.size());

                    // for (std::vector<CriteriaProgressTree*>::iterator itr = treeProgress->ChildrenTree.begin(); itr != treeProgress->ChildrenTree.end(); ++itr)
                        // TC_LOG_DEBUG("criteria.achievement", "AchievementInfo ChildrenTree criteriaTree %u parent %u", (*itr)->criteriaTree->ID, (*itr)->criteriaTree->Parent);
                    ...
                    // {
                        ...
                        // if(!progress)
                        // {
                            // TC_LOG_DEBUG("criteria.achievement", "AchievementInfo ChildrenCriteria error progress achievement %u", achievement->ID);
                            // continue;
                        // }
                        // CriteriaEntry const* criteria = progress->criteria;
                        // if(!criteria)
                        // {
                            // TC_LOG_DEBUG("criteria.achievement", "AchievementInfo ChildrenCriteria error criteria achievement %u criteriaTree %u parent %u", achievement->ID, progress->criteriaTree->ID, progress->parent->ID);
                            // continue;
                        // }
                        // TC_LOG_DEBUG("criteria.achievement", "AchievementInfo ChildrenCriteria criteria achievement %u criteriaTree %u parent %u criteria %u completed %i deactiveted %i",
        ...
        // }
        // else
            TC_LOG_DEBUG("criteria.achievement", "AchievementInfo error achievement %u not found", achievementId);
    ...
    static bool HandleAchievementDeleteCommand(ChatHandler* handler, char const* args)
    {
        ...
        TC_LOG_DEBUG("criteria.achievement", "HandleAchievementDeleteCommand achievmentID %i", achievmentID);
        if (!achievmentID)
            return false;
