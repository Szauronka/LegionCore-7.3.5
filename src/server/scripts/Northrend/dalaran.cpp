        ...
        void ReceiveEmote(Player* player, uint32 textEmote) override
        {
            if (textEmote == TEXT_EMOTE_SALUTE)
            {
                if (sGameEventMgr->IsActiveEvent(70))
                {
                    ...
                    {
                        DoCast(player, 231163);
                        me->HandleEmoteCommand(EMOTE_ONESHOT_SALUTE);
                    }
                    else
                    {
                        DoCast(player, 231193);
                        me->HandleEmoteCommand(EMOTE_ONESHOT_SALUTE);
                    }
                }
            }
...
struct npc_vormu_130654 : public ScriptedAI
{
    npc_vormu_130654(Creature* creature) : ScriptedAI(creature) { }
    ...
    void sGossipSelect(Player* player, uint32 /*sender*/, uint32 action) override
    {
        ...
        if (!player->GetGroup() || player->GetGroup()->GetLeaderGUID() == player->GetGUID())
            sLFGMgr->JoinLfg(player, LFG_DUNGEON_TIME_WALKING_ULDUAR);
    }
};
...
void AddSC_dalaran()
{
    ...
    RegisterCreatureAI(npc_vormu_130654);
}
