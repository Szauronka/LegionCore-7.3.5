    ...
    void DoAction(int32 const actionId) override
    {
        if (actionId == ACTION_ENCOUNTER_COMPLETE)
        {
            if (instance->GetBossState(DATA_EONAR) != DONE)
            {
                ...
                if (auto paraxisShip = instance->instance->GetGameObject(instance->GetGuidData(GO_EONAR_PARAXIS_SHIP)))
                    paraxisShip->SetVisible(false);
...
struct npc_eonar_the_paraxis : public ScriptedAI
{
    ...
    void JustDied(Unit* killer) override
    {
        ...
        if (auto owner = me->GetAnyOwner())
        {
            ...
            else
            {
                ...
                instance->DoUpdateAchievementCriteria(CRITERIA_TYPE_KILL_CREATURE, me->GetEntry(), 1, 0, me);

                if (auto player = MS::ScriptUtils::SelectRandomPlayerIncludedTank(me))
                    me->GetMap()->UpdateEncounterState(ENCOUNTER_CREDIT_KILL_CREATURE, me->GetEntry(), me, player);
                else
    ...
    void SummonedCreatureDies(Creature* summon, Unit* /*killer*/) override
    {
        ...
        if (me->GetAnyOwner())
        {
            if (auto eonar = me->GetAnyOwner()->ToCreature())
            {
                if (eonar->HasUnitState(UNIT_STATE_CASTING))
                    return;

                eonar->SetPower(POWER_ALTERNATE, eonar->GetPower(POWER_ALTERNATE) + energyCount);

                if (eonar->GetPower(POWER_ALTERNATE) >= 100)
                {
                    eonar->CastSpell(eonar, SPELL_LIFE_FORCE);

                    if (lifeForceCount < 4)
                    {
                        eonar->AI()->ZoneTalk(SAY_LIFE_FORCE_1 + lifeForceCount);
                        ++lifeForceCount;
                    }
                }
            }
    ...
    void SpellHitTarget(Unit* target, SpellInfo const* spell) override
    {
        ...
        {
            ZoneTalk(SAY_WARN_RAIN_OF_FEL, target->GetGUID());
        ...
        switch (portalCounter)
        {
            case 1: // MID
            {
                if (IsMythicRaid())
                {
                    ...
                    for (uint8 i = 0; i < 8; ++i)
                    {
                        ...
                        SummonCreatureDelay(delayTimer, NPC_FELGUARD, trashPos[0], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000);
                    }

                    SummonCreatureDelay(delayTimer + 4000, NPC_FEL_INFUSED_DESTRUCTOR, trashPos[0], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000);
                    delayTimer += 9000;
                    ...
                    for (uint8 i = 0; i < 12; ++i)
                    {
                        ...
                        SummonCreatureDelay(delayTimer, NPC_FEL_HOUND, trashPos[0], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000);
                    }
                }
                else
                {
                    ...
                    SummonCreatureDelay(delayTimer, NPC_FEL_INFUSED_DESTRUCTOR, trashPos[0], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000);
                }
            ...
            case 2:
            {
                if (IsMythicRaid()) //BOT+TOP
                {
                    ...
                    for (uint8 i = 0; i < 12; ++i)
                    {
                        ...
                        SummonCreatureDelay(delayTimer, NPC_FEL_HOUND, trashPos[1], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000);
                    }
                    SummonCreatureDelay(28000, NPC_FEL_CHARGED_OBFUSCATOR, trashPos[1], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000);

                    //>TOP
                    SummonShip(2, 24000);
                    SummonCreatureDelay(19000, NPC_FEL_LORD, trashPos[2], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000);
                    SummonCreatureDelay(21000, NPC_FEL_INFUSED_DESTRUCTOR, trashPos[2], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000);
                ...
                else // BOT
                {
                    ...
                    SummonCreatureDelay(delayTimer, NPC_FEL_INFUSED_DESTRUCTOR, trashPos[1], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000);
            ...
            case 3:
            {
                if (IsMythicRaid()) //MID
                {
                    ...
                    SummonCreatureDelay(delayTimer + 1000, NPC_FEL_POWERED_PURIFIER, trashPos[0], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000);
                    events.RescheduleEvent(EVENT_SUMMON_SHIP, 23500);
                }
                else //TOP
                {
                    SummonShip(2, 27000);
                    SummonCreatureDelay(10000, NPC_FEL_LORD, trashPos[2], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000);
                    SummonCreatureDelay(15000, NPC_FEL_LORD, trashPos[2], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000);
                    SummonCreatureDelay(20000, NPC_FEL_CHARGED_OBFUSCATOR, trashPos[2], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000);
                    SummonCreatureDelay(25000, NPC_FEL_INFUSED_DESTRUCTOR, trashPos[2], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000);
            ...
            case 4:
            {
                if (IsMythicRaid()) //TOP
                {
                    ...
                    for (uint8 i = 0; i < 12; ++i)
                    {
                        ...
                        SummonCreatureDelay(delayTimer, NPC_FEL_HOUND, trashPos[2], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000);
                    }
                ...
                else // MID
                {
                    ...
                    if (IsHeroicPlusRaid())
                        SummonCreatureDelay(delayTimer + 500, NPC_FEL_POWERED_PURIFIER, trashPos[0], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000);
            ...
            case 5:
            {
                ...
                {
                    SummonShip(1, 38000);
                    SummonCreatureDelay(8000, NPC_FEL_LORD, trashPos[1], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000);
                    SummonCreatureDelay(13000, NPC_FEL_LORD, trashPos[1], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000);
                ...
                else // TOP+BOT
                {
                    ...
                    for (uint8 i = 0; i < 6; ++i)
                    {
                        ...
                        SummonCreatureDelay(delayTimer, NPC_FELGUARD, trashPos[2], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000);
                    }
                    //>BOTTOM
                    delayTimer = 25000;
                    for (uint8 i = 0; i < 5; ++i)
                    {
                        ...
                        SummonCreatureDelay(delayTimer, NPC_FELGUARD, trashPos[1], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000);
                    }
                    SummonCreatureDelay(20000, NPC_FEL_INFUSED_DESTRUCTOR, trashPos[1], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000);
                    if (IsHeroicRaid())
                        SummonCreatureDelay(20000, NPC_FEL_POWERED_PURIFIER, trashPos[1], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000);
            ...
            case 6:
            {
                ...
                {
                    SummonShip(0, 55000);
                    SummonCreatureDelay(20000, NPC_FEL_LORD, trashPos[0], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000);
                    SummonCreatureDelay(25000, NPC_FEL_LORD, trashPos[0], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000);
                    SummonCreatureDelay(30000, NPC_FEL_POWERED_PURIFIER, trashPos[0], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000);
                ...
                else // MID
                {
                    SummonShip(0, 42000);
                    SummonCreatureDelay(25000, NPC_FEL_LORD, trashPos[0], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000);
                    SummonCreatureDelay(30000, NPC_FEL_CHARGED_OBFUSCATOR, trashPos[0], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000);
                    SummonCreatureDelay(35000, NPC_FEL_LORD, trashPos[0], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000);
                    delayTimer = 27000;
                    for (uint8 i = 0; i < 6; ++i)
                    {
                        if (i == 2 || i == 4)
                    ...
                    if (IsHeroicRaid())
                        SummonCreatureDelay(20000, NPC_FEL_POWERED_PURIFIER, trashPos[0], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000);
            ...
            case 8:
            {
                if (IsMythicRaid()) //MID+BOT
                {
                    ...
                    SummonShip(0, 55000);
                    SummonCreatureDelay(20000, NPC_FEL_LORD, trashPos[0], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000);
                    SummonCreatureDelay(25000, NPC_FEL_CHARGED_OBFUSCATOR, trashPos[0], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000);
                    SummonCreatureDelay(30000, NPC_FEL_LORD, trashPos[0], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000);
                    //BOT
                    SummonShip(1, 55000);
                    delayTimer = 40000;
                    for (uint8 i = 0; i < 12; ++i)
                ...
                else //ALL
                {
                    ...
                    {
                        SummonShip(i, 48000);
                        SummonCreatureDelay(delayTimer, NPC_FEL_LORD, trashPos[i], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000);
                        SummonCreatureDelay(delayTimer + 5000, i == 1 ? NPC_FEL_CHARGED_OBFUSCATOR : NPC_FEL_INFUSED_DESTRUCTOR, trashPos[i], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000);
                    }
                }
                break;
            }
            case 9: //TOP
                SummonShip(2, 45000);
                delayTimer = 20000;
                SummonCreatureDelay(delayTimer, NPC_FEL_CHARGED_OBFUSCATOR, trashPos[2], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000);
                for (uint8 i = 0; i < 6; ++i)
                {
                    if (!(i % 2))
                        delayTimer += 1000;
            ...
            case 10: //TOP
                SummonShip(2, 55000);
                SummonCreatureDelay(40000, NPC_FEL_LORD, trashPos[2], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000);
                SummonCreatureDelay(45000, NPC_FEL_INFUSED_DESTRUCTOR, trashPos[2], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000);
                SummonCreatureDelay(50000, NPC_FEL_LORD, trashPos[2], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000);
                events.RescheduleEvent(EVENT_SUMMON_SHIP, 100000); //1m40s
                break;
            case 11: //ALL
                delayTimer = 20000;
        ...
        switch (flyPortalCounter)
        {
            case 1:
                if (IsMythicRaid())
                {
                    ...
                    for (uint8 i = 0; i < 6; ++i)
                    {
                        SummonCreatureDelay(delayTimer, NPC_VOLANT_KERAPTERON, trashPos[3], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000);
                        delayTimer += 1000;
                    }
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            switch (eventId)
            {
                ...
                case EVENT_SPEAR_OF_DOOM:
                    ...
                    {
                        ZoneTalk(SAY_WARN_SPEAR_OF_DOOM);
...
struct npc_eonar_trash_generic : public ScriptedAI
{
    npc_eonar_trash_generic(Creature* creature) : ScriptedAI(creature)
    {
        ...
        switch (me->GetEntry())
        {
            ...
            case NPC_VOLANT_KERAPTERON:
                ...
                me->ApplySpellImmune(0, IMMUNITY_MECHANIC, MECHANIC_INCAPACITATE, true);
                break;
        }
    ...
    void LastWPReached() override
    {
        ...
        me->CastSpellDelay(me, SPELL_STRIKE, true, 500);
    }

    void OnApplyOrRemoveAura(uint32 spellId, AuraRemoveMode mode, bool apply) override
    {
        if (spellId == SPELL_CLOAK_MOD_DMG)
        {
            if (apply)
                me->SetFlag(UNIT_FIELD_FLAGS_2, UNIT_FLAG2_PREVENT_SELECT_NPC);
            else
        ...
        for (uint8 i = 0; i < 4; ++i)
        {
            if (me->GetDistance(trashPos[i]) < 10.0f)
            {
                ...
                else
                {
                    ...
                    me->GetMotionMaster()->MovePath(pathID[i], false, frand(-2.0f, 2.0f), frand(-2.0f, 2.0f));
                }
                return;
            }
        }
    ...
    void UpdateAI(uint32 diff) override
    {
        if (checkSpellTimer)
        {
            if (checkSpellTimer <= diff)
            {
                ...
                {
                    if (!(me->GetCurrentSpell(CURRENT_CHANNELED_SPELL) && me->GetCurrentSpell(CURRENT_CHANNELED_SPELL)->m_spellInfo->Id == SPELL_STRIKE))
                    {
            ...
            if (checkTargetTimer)
            {
                if (checkTargetTimer <= diff)
                {
                    ...
                    if (auto victim = me->getVictim())
                    {
                        ...
                        {
                            if (auto target = SelectTarget(SELECT_TARGET_NEAREST, 0, 60.0f, true))
                            {
...
struct npc_eonar_spear_of_doom : public ScriptedAI
{
    ...
    void IsSummonedBy(Unit* summoner) override
    {
        ...
        summoner->CastSpellDelay(me, SPELL_SPEAR_OF_DOOM_AT, true, 500);
    ...
    void UpdateAI(uint32 diff) override
    {
        if (moveTimer)
        {
            if (moveTimer <= diff)
            {
                ...
                else if (auto player = Player::GetPlayer(*me, targetGUID))
                {
    ...
    void UpdateAI(uint32 diff)
    {
        ...
        if (tormentTimer)
        {
            if (tormentTimer <= diff)
            {
                ...
                if (me->FindNearestPlayer(50.0f, true))
                {
...
class spell_eonar_spear_of_doom_filter : public SpellScript
{
    ...
    void FilterTargets(std::list<WorldObject*>& targets)
    {
        ...
        targets.remove_if([this](WorldObject* object) -> bool
        {
            if (object == nullptr || !object->IsPlayer() || object->GetCurrentAreaID() != 9333)
                return true;
            return false;
        });
...
class spell_eonar_rain_of_fel_filter : public SpellScript
{
    ...
    void FilterTargets(std::list<WorldObject*>& targets)
    {
        ...
        if (!tempTargets.empty())
        {
            ...
            {
                Trinity::Containers::RandomResizeList(targets, 3 - tempTargets.size());
...
class spell_eonar_paraxis_teleport : public SpellScript
{
    ...
    void Register()
    {
        OnEffectLaunch += SpellEffectFn(spell_eonar_paraxis_teleport::HandleDummy, EFFECT_0, SPELL_EFFECT_TELEPORT_L);
    }
};
...
class spell_eonar_jump_pad : public SpellScript
{
    ...
    void HandleDummy(SpellEffIndex /*effectIndex*/)
    {
        ...
        padList.sort(Trinity::UnitSortDistance(true, caster));
    ...
    void Register()
    {
        OnEffectLaunch += SpellEffectFn(spell_eonar_jump_pad::HandleDummy, EFFECT_0, SPELL_EFFECT_SMASH_DEST);
    }
};
...
class spell_eonar_dive_down_jump : public SpellScript
{
    ...
    void HandleScript(SpellEffIndex /*effectIndex*/)
    {
        ...
        caster->AddDelayedEvent(1000, [caster]() -> void { caster->CastSpellDuration(caster, SPELL_SWIFT_FEET, true, 6000, 3); });
    }

    void Register() override
    {
        OnEffectHit += SpellEffectFn(spell_eonar_dive_down_jump::HandleScript, EFFECT_0, SPELL_EFFECT_JUMP_DEST);
        OnEffectHit += SpellEffectFn(spell_eonar_dive_down_jump::HandleScript, EFFECT_0, SPELL_EFFECT_SMASH_DEST);
    }
};
...
class spell_eonar_swift_feet : public AuraScript
{
    ...
    void Register() override
    {
        OnEffectUpdate += AuraEffectUpdateFn(spell_eonar_swift_feet::OnUpdate, EFFECT_0, SPELL_AURA_MOD_SPEED_NOT_STACK);
    }
};
