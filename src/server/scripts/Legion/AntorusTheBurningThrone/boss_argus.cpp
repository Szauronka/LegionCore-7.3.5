...
struct boss_argus : BossAI
{
    ...
    void EnterEvadeMode() override
    {
        ...
        me->SummonCreatureGroupDespawn(SUMMON_GROUP_HUNGERING_SOUL);
    ...
    void DamageTaken(Unit* /*attacker*/, uint32& damage, DamageEffectType dmgType) override
    {
        if (me->HealthBelowPct(healthPct))
        {
            ...
            if (phase == PHASE_2)
            {
                if (auto amanthul = instance->GetCreatureByEntry(NPC_ARGUS_AMANTHUL))
                {
                    amanthul->AI()->ZoneTalk(SAY_PHASE_2);
                }
                if (auto golganneth = instance->GetCreatureByEntry(NPC_ARGUS_GOLGANNETH))
                {
                    golganneth->CastSpell(golganneth, SPELL_GOLGANNETH_WRATH_DUMMY, true);
                }
                me->AddDelayedCombat(5000, [this]() -> void { me->RemoveAllAreaObjects(); });
    ...
    void MovementInform(uint32 type, uint32 id) override
    {
        ...
        if (id == 1)
        {
            ...
            if (auto khazgoroth = instance->GetCreatureByEntry(NPC_ARGUS_KHAZGOROTH))
                me->AddThreat(khazgoroth, 1.0f);
        }
    }
    ...
    void SpellFinishCast(const SpellInfo* spell) override
    {
        switch (spell->Id)
        {
            case SPELL_GRASP_OF_THE_UNMAKER:
                ZoneTalk(SAY_GRASP_OF_THE_UNMAKER);
                break;
            case SPELL_REAP_SOUL_DUMMY:
            {
                if (auto amanthul = instance->GetCreatureByEntry(NPC_ARGUS_AMANTHUL))
                {
                    amanthul->AI()->ZoneTalk(SAY_REAP_SOUL);
                }
                if (auto khazgoroth = instance->GetCreatureByEntry(NPC_ARGUS_KHAZGOROTH))
                {
                ...
                me->CastSpellDelay(me, SPELL_REAP_SOUL_KILL_PLAYERS, true, 2000);
                break;
            }
            case SPELL_REAP_SOUL_KILL_PLAYERS:
                me->CastSpellDelay(me, SPELL_P4_ARCANE_DISSOLVE_OUT, false, 2000);
    ...
    void SpellHit(Unit* caster, SpellInfo const* spell) override
    {
        if (spell->Id == SPELL_TEMPORAL_BLAST)
        {
            ...
            if (IsMythicRaid())
            {
                ...
                me->CastSpellDelay(me, SPELL_P3_ARCANE_DISSOLVE_OUT, true, 3000);
                return;
            }
            else
    ...
    void SpellHitTarget(Unit* target, SpellInfo const* spell) override
    {
        switch (spell->Id)
        {
            ...
            case SPELL_SOULBURST:
                ...
                break;
            case SPELL_SOULBOMB:
                Talk(SAY_WARN_WHISPER_SOULBOMB, target->GetGUID());
                break;
        }
    }
    ...
    void PrepareEventsPhaseFour()
    {
        ...
        me->CastSpellDelay(me, SPELL_REAP_SOUL_DUMMY, false, 1000);
        me->CastSpellDelay(me, SPELL_GRASP_OF_THE_UNMAKER, true, 4500);
        me->AddDelayedCombat(10000, [this]() -> void
        {
    ...
    void EdgeAnnihilation()
    {
        ...
        for (uint8 i = 0; i < 16; ++i)
        {
            ...
            if (i < 8)
                ...
                SummonCreatureDelay(5000, NPC_EDGE_OF_ANNIHILATION, pos);
        }
    }

    uint32 GetData(uint32 type) const override
    {
        if (type == DATA_SARGERAS_GAZE)
            return sargerasGazeCount;
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (!prePhaseFour && CheckHomeDistToEvade(diff, 50.0f, centrPos.GetPositionX(), centrPos.GetPositionY(), centrPos.GetPositionZ()))
            return;
        ...
        if (uint32 eventId = events.ExecuteEvent())
        {
            switch (eventId)
            {
                ...
                case EVENT_SWEEPING_SCYTHE:
                    if (IsHeroicRaid())
                        DoCastTopAggro(SPELL_DEADLY_SCYTHE);
                    else
                        DoCastTopAggro(SPELL_SWEEPING_SCYTHE);
                ...
                case EVENT_SKY_AND_SEA:
                    if (auto amanthul = instance->GetCreatureByEntry(NPC_ARGUS_AMANTHUL))
                    {
                        amanthul->AI()->ZoneTalk(SAY_WARN_SKY_AND_SEA);
                        if (!saySkyAndSeaSays)
                        {
                            saySkyAndSeaSays = true;
                            amanthul->AI()->ZoneTalk(SAY_SKY_AND_SEA);
                        }
                    }
                    if (auto golganneth = instance->GetCreatureByEntry(NPC_ARGUS_GOLGANNETH))
                ...
                case EVENT_AVATAR_OF_AGGRAMAR:
                    if (!sayAvatarOfAggramar)
                    {
                        ...
                        if (auto amanthul = instance->GetCreatureByEntry(NPC_ARGUS_AMANTHUL))
                            amanthul->AI()->ZoneTalk(SAY_AVATAR_OF_AGGRAMAR);
                    }
                ...
                case EVENT_TEMPORAL_BLAST:
                    if (auto amanthul = instance->GetCreatureByEntry(NPC_ARGUS_AMANTHUL))
                    {
                        amanthul->AI()->ZoneTalk(SAY_TEMPORAL_BLAST);
                ...
                case EVENT_COSMIC_BEACON:
                {
                    ...
                    Trinity::Containers::RandomResizeList(tempList, IsHeroicPlusRaid() ? 3 : 2);
                ...
                {
                    std::list<ObjectGuid> tempList = constellarList;
                    Trinity::Containers::RandomResizeList(tempList, 2);
                ...
                case EVENT_THE_DISCS_OF_NORGANNON:
                    if (auto norgannon = instance->GetCreatureByEntry(NPC_ARGUS_NORGANNON))
                        norgannon->CastSpell(me, SPELL_THE_DISCS_OF_NORGANNON);
                    break;
                case EVENT_GIFT_OF_THE_LIFEBINDER:
                    if (auto eonar = instance->GetCreatureByEntry(NPC_ARGUS_EONAR))
                    {
                        eonar->AI()->ZoneTalk(SAY_GIFT_OF_THE_LIFEBINDER);
                ...
                case EVENT_END_OF_ALL_THINGS:
                {
                    ...
                    me->AddDelayedCombat(1000, [this]() -> void
                    {
                        ...
                        instance->SendEncounterUnit(ENCOUNTER_FRAME_SET_ALLOWING_RELEASE, me, true);
                    });
                    me->AddDelayedCombat(4000, [this]() -> void
                    {
                        me->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE | UNIT_FLAG_IMMUNE_TO_PC | UNIT_FLAG_IMMUNE_TO_NPC | UNIT_FLAG_NOT_ATTACKABLE_1);
                ...
                case EVENT_SARGERAS_GAZE:
                    if (sargerasGazeCount < 8)
                        ++sargerasGazeCount;
                    ZoneTalk(SAY_WARN_SARGERAS_GAZE);
                    me->CastSpell(me, SPELL_MYTHIC_P1_CLEANUP, true);
                    me->CastSpellDelay(me, SPELL_SARGERAS_GAZE, true, 2500);
                ...
                case EVENT_SOULRENDING_SCYTHE:
                    DoCastTopAggro(SPELL_SOULRENDING_SCYTHE);
...
struct npc_argus_titans_generic : ScriptedAI
{
    ...
    void SpellFinishCast(const SpellInfo* spell) override
    {
        switch (spell->Id)
        {
            ...
            case SPELL_GIFT_OF_THE_LIFEBINDER:
                ZoneTalk(SAY_GIFT_OF_THE_LIFEBINDER_2);
                break;
        }
    }
    ...
    void SpellHitTarget(Unit* target, SpellInfo const* spell) override
    {
        if (spell->Id == SPELL_GIFT_OF_THE_SKY_AURA)
            ZoneTalk(SAY_WARN_WHISPER_SKY, target->GetGUID());

        if (spell->Id == SPELL_GIFT_OF_THE_SEA_AURA)
            ZoneTalk(SAY_WARN_WHISPER_SEA, target->GetGUID());
    }
    ...
    void OnRemoveAuraTarget(Unit* target, uint32 spellId, AuraRemoveMode mode) override
    {
        ...
        switch (spellId)
        {
            case SPELL_GIFT_OF_THE_SKY_AURA:
                if (auto owner = me->GetAnyOwner())
                    me->CastSpell(target, SPELL_GIFT_OF_THE_SKY_MISSILE, true, nullptr, nullptr, owner->GetGUID());
                break;
            case SPELL_GIFT_OF_THE_SEA_AURA:
                if (auto owner = me->GetAnyOwner())
                    me->CastSpell(target, SPELL_GIFT_OF_THE_SEA_MISSILE, true, nullptr, nullptr, owner->GetGUID());
                break;
        }
    }
...
struct npc_argus_soulblight_orb : ScriptedAI
{
    ...
    void SpellHitTarget(Unit* target, SpellInfo const* spell) override
    {
        if (spell->Id == SPELL_SOULBLIGHT_DOT)
            ZoneTalk(SAY_WARN_SOULBLIGHT_DOT, target->GetGUID());
    }
    ...
    void OnApplyOrRemoveAura(uint32 spellId, AuraRemoveMode mode, bool apply) override
    {
        ...
        {
            if (auto owner = me->GetAnyOwner())
                me->CastSpell(me, SPELL_DEATH_FOG_AT, true, nullptr, nullptr, owner->GetGUID());
...
struct npc_argus_reorigination_module : ScriptedAI
{
    ...
    void IsSummonedBy(Unit* summoner) override
    {
        if (isMythicModule)
        {
            me->CastSpellDelay(me, SPELL_APOCALYPSIS_MODULE_AT, true, 1500);
            me->CastSpellDelay(me, SPELL_APOCALYPSIS_MODULE_PERIODIC, true, 1500);
        }
        else
        {
        ...
        switch (spell->Id)
        {
            ...
            case SPELL_APOCALYPSIS_MODULE_REMOVE:
            {
                if (isMythicModule)
                {
                    ...
                    {
                        if (me)
                            if (auto at = me->GetAreaObject(SPELL_APOCALYPSIS_ZONE_AT))
                                at->SetSphereScale(apocalypsisSize, 1000, true);
                    });
                }
                else
                {
    ...
    void ReoriginationPulse()
    {
        ...
        if (isMythicModule)
        {
            ...
            if (auto at = me->GetAreaObject(SPELL_APOCALYPSIS_MODULE_AT))
                apocalypsisSize = at->GetRadius();
        }
        else
        {
...
struct npc_argus_chains_of_sargeras : ScriptedAI
{
    ...
    void IsSummonedBy(Unit* summoner) override
    {
        ...
        if (summoner)
            me->CastSpellDelay(summoner, SPELL_SENTENCE_OF_SARGERAS_DMG, true, 100);
    }
...
class spell_argus_p2_energize_periodic : public AuraScript
{
    ...
    void OnTick(AuraEffect const* aurEff)
    {
        ...
        if (powerCount < 100)
        {
            ...
            {
                effectValue = 2.0f;
                caster->CastCustomSpell(caster, SPELL_VOLATILE_SOUL_FILTER_2, &effectValue, nullptr, nullptr, true);
            }
        }
        else if (!caster->HasUnitState(UNIT_STATE_CASTING))
        {
            effectValue = 3.0f;
            caster->CastCustomSpell(caster, SPELL_VOLATILE_SOUL_FILTER, &effectValue, nullptr, nullptr, false);
        }
    }
...
class spell_argus_p4_energize_periodic : public AuraScript
{
    ...
    void OnTick(AuraEffect const* aurEff)
    {
        ...
        if (++tickCount == maxTick)
        {
            ...
            {
                effectValue = 3.0f;
                caster->CastCustomSpell(caster, SPELL_VOLATILE_SOUL_FILTER, &effectValue, nullptr, nullptr, true);
            }
            else
            {
                effectValue = 2.0f;
...
class spell_argus_initialization_sequence_periodic : public AuraScript
{
    ...
    void OnTick(AuraEffect const* aurEff)
    {
        ...
        if (auto at = GetCaster()->GetAreaObject(GetId() == SPELL_INITIALIZATION_SEQUENCE ? SPELL_REORIGINATION_MODULE_AT : SPELL_APOCALYPSIS_MODULE_AT))
        {
            at->SetSphereScale(dmgPct * 0.05f, 500, false);
            GetCaster()->CastCustomSpell(GetCaster(), SPELL_INITIALIZATION_SEQUENCE_SELF_DMG, &dmgPct, NULL, NULL, true);
        }
    }
...
class spell_argus_aggramar_boon : public AuraScript
{
    ...
    void Absorb(AuraEffect* aurEff, DamageInfo& dmgInfo, float& absorbAmount)
    {
        ...
        {
            absorbAmount = dmg;
            GetTarget()->CastCustomSpell(GetCaster(), SPELL_AGGRAMAR_SACRIFICE, &dmg, nullptr, nullptr, true);
        }
    }
    ...
    void Register() override
    {
        ...
        OnEffectAbsorb += AuraEffectAbsorbFn(spell_argus_aggramar_boon::Absorb, EFFECT_0, SPELL_AURA_SCHOOL_ABSORB);
    }
};
...
class spell_argus_titanforging_energize_periodic : public AuraScript
{
    ...
    void OnCheckTick(AuraEffect const* aurEff)
    {
        ...
        if (allowCast)
            caster->CastCustomSpell(caster, SPELL_TITANFORGED_BUFF, &amount, &amount, &amount, &amount, &amount, &amount, true);
    }
...
class spell_argus_sky_and_sea : public SpellScript
{
    ...
    void FilterTargets(std::list<WorldObject*>& targets)
    {
        ...
        {
            if (object == nullptr || !object->IsPlayer() || object->ToPlayer()->isInTankSpec())
                return true;
...
class spell_argus_golganneth_wrath : public SpellScript
{
    ...
    void HandleScript(SpellEffIndex /*effectIndex*/)
    {
        ...
        GetCaster()->CastSpell(GetHitDest(), SPELL_GOLGANNETH_WRATH_PERSISTENT, false);
    }
...
class spell_argus_golganneth_wrath_filter : public SpellScript
{
    ...
    void Register() override
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_argus_golganneth_wrath_filter::FilterTargets, EFFECT_0, TARGET_UNIT_SRC_AREA_ENTRY);
    }
};
...
class spell_argus_soul_detonation : public SpellScript
{
    ...
    void HandleDamage(SpellEffIndex /*effIndex*/)
    {
        ...
        float maxDist = GetCaster()->GetMap()->IsHeroicPlusRaid() ? 45.0f : 35.0f;
...
//256396
class spell_argus_reorigination_pulse : public SpellScript
{
    PrepareSpellScript(spell_argus_reorigination_pulse);
    ...
    void FilterTargets(std::list<WorldObject*>& targets)
    {
        ...
        targets.remove_if([caster](WorldObject* object) -> bool
        {
            ...
            if (auto at = caster->GetAreaObject(SPELL_REORIGINATION_MODULE_AT))
                if (at->GetDistance(object) > at->GetRadius())
                    return true;
    ...
    void Register() override
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_argus_reorigination_pulse::FilterTargets, EFFECT_0, TARGET_UNIT_DEST_AREA_ENEMY);
    }
};
...
class spell_argus_sargeras_gaze : public SpellScript
{
    ...
    void FilterTargets(std::list<WorldObject*>& targets)
    {
        ...
        targets.remove_if([this](WorldObject* object) -> bool
        {
            ...
            {
                if (player->IsRangedDamageDealer(true))
                    rangedList.push_back(object);
                else if (player->IsMeleeDamageDealer(false))
...
//Custom: 16778 (Entry: 12071)
struct at_argus_sargeras_fear : AreaTriggerAI
{
    at_argus_sargeras_fear(AreaTrigger* areatrigger) : AreaTriggerAI(areatrigger) {}

    void OnRemove() override
    {
        if (auto owner = at->GetCaster())
            owner->RemoveAurasDueToSpell(SPELL_SARGERAS_FEAR_VISUAL);
    }
    ...
    void ActionOnUpdate(GuidList& affectedPlayers) override
    {
        ...
        if (affectedPlayers.empty() && !owner->HasAura(SPELL_SARGERAS_FEAR_VISUAL))
        {
            owner->CastSpell(owner, SPELL_SARGERAS_FEAR_VISUAL, true);
        }
        else if (!affectedPlayers.empty() && owner->HasAura(SPELL_SARGERAS_FEAR_VISUAL))
        {
            owner->RemoveAurasDueToSpell(SPELL_SARGERAS_FEAR_VISUAL);
        }

        if (affectedPlayers.empty())
            owner->CastSpell(owner, SPELL_SARGERAS_FEAR_DMG, true);
    }
};

//Custom: 16883 (Entry: 12185)
struct at_argus_rent_soul : AreaTriggerAI
{
    at_argus_rent_soul(AreaTrigger* areatrigger) : AreaTriggerAI(areatrigger) {}

    uint32 timerStart = 2000;

    void ModifyMoveSpeed(float& speed) override
    {
        if (speed < 50.0f)
            speed += 0.5f;
    }
    ...
    void OnUpdate(uint32 diff) override
    {
        if (timerStart)
        {
            if (timerStart <= diff)
            {
                ...
                timerStart = 1000;
                at->ReCalculateSplinePosition(true);
            }
            else
                timerStart -= diff;
        }
...
void AddSC_boss_argus()
{
    ...
    RegisterSpellScript(spell_argus_reorigination_pulse);
